...
from .lsnet import LSConv, Block as LSBlock
...
class C3k2_pac(C3k2):
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        """Initializes the C3k2 module, a faster CSP Bottleneck with 2 convolutions and optional C3k blocks."""
        super().__init__(c1, c2, n, c3k, shortcut, g, e)
        self.m = nn.ModuleList(
            C3k_pac(self.c, self.c, 2, shortcut, g) if c3k else ParallelAtrousConv(self.c, ratio=[1, 2, 3]) for _ in range(n)
        )
...
class C3k_pac(C3k):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, k=3):
        ...
        c_ = int(c2 * e)  # hidden channels
        # self.m = nn.Sequential(*(RepBottleneck(c_, c_, shortcut, g, k=(k, k), e=1.0) for _ in range(n)))
        self.m = nn.Sequential(*(ParallelAtrousConv(c_, ratio=[1, 2, 3]) for _ in range(n)))

class ParallelAtrousConv(nn.Module):
    def __init__(self, inc, ratio=[1, 2, 3]) -> None:
        super().__init__()
        self.repconv = RepConvN(inc, inc, k=3)
    ...
    def forward(self, x):
        return x + self.conv4(
            torch.cat([self.conv1(self.repconv(x)), self.conv2(self.repconv(x)), self.conv3(self.repconv(x))], dim=1))
...
class C3k2_pms(C3k2):
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        super().__init__(c1, c2, n, c3k, shortcut, g, e)
        self.m = nn.ModuleList(
            C3k_pms(self.c, self.c, 2, shortcut, g) if c3k else PMSFA(self.c) for _ in range(n)
        )
...
class C3k2_lsblock(C3k2):
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        super().__init__(c1, c2, n, c3k, shortcut, g, e)
        self.m = nn.ModuleList(
            C3k_lsblock(self.c, self.c, 2, shortcut, g) if c3k else LSBlock(self.c) for _ in range(n)
        )
